...
const HeroSection = ({ product: propProduct }) => {
 ...
 const isDark = theme.palette.mode === 'dark';
  ...
  // Get wishlist tools from context
  ...
  const [activeProduct, setActiveProduct] = useState(() => {
    return propProduct || sampleData.find((item) => item.id === Number(id)) || sampleData[0];
  });

  // Check global wishlist status instead of local state
  ...
  // 2. Sync state when ID or Cart changes
  useEffect(() => {
    const newProduct = sampleData.find((item) => item.id === Number(id));
    if (newProduct) {
      setActiveProduct(newProduct);
      setIsAdded(cartItems.some(item => item.id === newProduct.id));
    }
  }, [id, cartItems]);
...
>
  {/* Filter sampleData to show products from the same category */}
  {sampleData
    .filter((item) => item.category === activeProduct.category)
    .slice(0, 5) // Limits to 5 related items
    .map((item) => (
      <Box
        key={item.id}
  ...
  // Use toggleWishlist to update the global state and count
