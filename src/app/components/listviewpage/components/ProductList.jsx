...
const ProductList = ({ activeFilters = [], priceRange = [0, 5000] }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const categoryQuery = searchParams.get('category');

  // Helper to make title look clean: "mobiles" -> "Mobiles"
  ...
  const filteredProducts = sampleData.filter(product => {
    ...
    // 2. Logic: Sidebar filters active
    ...
    // 3. Logic: Default Category from URL
    if (formattedQuery) {
